...
**Status:** Fully built and functional, runs locally or via Docker. The backend is deployment-ready (Dockerized, CPU-optimized) but not currently hosted live — CLIP/PyTorch's memory footprint exceeds the 512MB cap on free-tier hosts (Render, Railway), and a paid tier was out of scope for this build. Getting it live is a hosting-config change, not a code change.
...
This is a complete, working application — clone it, run the two commands below, and it's fully usable end to end (signup, post items, get matched, claim, verify, exchange contact).
...
cp .env.example .env              # fill in DATABASE_URL, SECRET_KEY, ALLOWED_EMAIL_DOMAINS
...
**Or, run the backend in Docker** (same environment it'd deploy in):
```bash
docker build -t lost-found-matcher ./backend
docker run -p 8000:8000 --env-file backend/.env lost-found-matcher
```
...
Built by Sid, CSE @ RV University.
